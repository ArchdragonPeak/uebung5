...
public class BaelleView implements Observer {
	...
	public BaelleView(BaelleControl baelleControl, Stage primaryStage, BaelleModel baelleModel) {
	...
	private void initKomponenten() {
		...
		pane.getChildren().addAll(lblAenderung, lblEinkaufsdatum, lblMaterial, lblSportart, lblStatus, lblNutzung,
				lblPreis);
		...
		pane.getChildren().addAll(cmbBxEinkaufdatum, txtMaterial, txtSportart, txtStatus, txtNutzung, txtPreis);
	...
	private void zeigeBallAn() {
		...
		if (einkaufsdatum == null) {
			this.zeigeInformationsfensterAn("W�hlen Sie ein Einkaufsdatum aus.");
		} else {
...
// public void update(Observable obs, Object obj){
	public void update() {
		...
		for (int i = 0; i < baelleModel.getBaelle().getAnzahlSportartikel(); i++) {
			cmbBxEinkaufdatum.getItems().add(baelleModel.getBaelle().getSportartikel(i).getEinkaufsdatum() + "");
		}
	}
